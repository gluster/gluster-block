...
glusterBlockDParseArgs(int count, char **options)
{
  ...
  while (optind < count) {
    ...
    case GB_DAEMON_GLUSTERD_HOST:
      if (count - optind  < 1) {
        MSG(stderr, "option '%s' needs argument <hostname>\n",
            options[optind-1]);
        return -1;
      }
      if (GB_STRDUP(gbConf.glusterdHostname, options[optind]) < 0) {
        MSG(stderr, "failed to copy hostname: %s\n", options[optind]);
        return -1;
      }
      break;
    ...
    case GB_DAEMON_GLUSTERD_PORT:
      if (count - optind  < 1) {
        MSG(stderr, "option '%s' needs argument <port>\n", options[optind-1]);
        return -1;
      }
      if (sscanf(options[optind], "%d", &gbConf.glusterdPort) != 1) {
        MSG(stderr, "failed to get port details: %s\n", options[optind]);
        return -1;
      }
      break;
    }
